...
export default function About() {
  ...
  useEffect(() => {
    axios.get("/api/about").then(res => {
      setAbout(res.data); // ✅ just use what backend sends
    });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-antique">
      ...
      {/* Hero Section */}
      <section className="py-20 bg-white nav-space">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div>
              ...
              <p className="text-lg text-gray-700 mb-6">{about?.paragraph1}</p>
              <p className="text-lg text-gray-700 mb-6">{about?.paragraph2}</p>
              <p className="text-lg text-gray-700 mb-8">{about?.paragraph3}</p>
  ...
  {Array(4).fill(about?.image_urls?.[0]).map((url: string | undefined, idx: number) => (
    <div
      key={idx}
      className={`h-48 bg-cover bg-center rounded-xl shadow-lg duration-300 hover:scale-105 
        ${idx === 1 ? "mt-8" : ""} 
        ${idx === 2 ? "-mt-8" : ""}`}
